...
if os.path.exists(base_path+expt_path1+'/progress_report'):
    ...
    d = {'run': prog_vec, 'script': ['timepoint_populate_cells_all_scenes_1', 'timepoint_populate_cells_all_scenes_2',
                                     'timepoint_track_localization_manual_annotation',
                                     'timepoint_analyze_whi5_distribution']}
    temp_df = pd.DataFrame(data=d)

# Step 1:
# producing the necessary lists of cells from the segmentation and tracking data. Assumes a file structure produced by
# make_image_dirs.py
if temp_df.loc[temp_df[temp_df.script == 'timepoint_populate_cells_all_scenes_1'].index[0], 'run'] == 0:
    # if this has not already been run
    C.timepoint_populate_cells_all_scenes_1(base_path, expt_paths, expt_conds, image_filenames, bf_filename, completed_scenes,
                                  num_scenes)
...
if temp_df.loc[temp_df[temp_df.script == 'timepoint_populate_cells_all_scenes_2'].index[0], 'run'] == 0:
    # if this has not already been run
    C.timepoint_populate_cells_all_scenes_2(base_path, expt_paths, expt_conds, image_filenames, bf_filename, completed_scenes,
                                  num_scenes, bkgd_scenes, fl_filename, fl_filename_c2)
...
if temp_df.loc[temp_df[temp_df.script == 'timepoint_track_localization_manual_annotation'].index[0], 'run'] == 0:
    # if this has not already been run
...
if temp_df.loc[temp_df[temp_df.script == 'timepoint_analyze_whi5_distribution'].index[0], 'run'] == 0:
    # if this has not already been run
