...
frame_nums = [31, 51, 51, 51, 51]  # different frame numbers for different scenes
for scene in range(1, 6):
    ...
    for frame_num in range(1, frame_nums[scene-1]):  # we go through frame by frame, starting with frame # 2 since we need a previous
        ...
        #         print c[ind1].position
        d_cell_coords = [c[ind1].position[frame_num - c[ind1].frames[0]] for ind1 in assigned_inds[1] if
                         len(c[ind1].position)>frame_num-c[ind1].frames[0]]
